...
const StepAppointment = ({
  ...
  // Auto-select January 2nd, 2026 if no date is provided
  const getInitialDate = () => {
    if (formData.selectedDate) return formData.selectedDate;
    const januarySecond = new Date(2026, 0, 2); // January 2nd, 2026 (month is 0-indexed)
    januarySecond.setHours(0, 0, 0, 0);
    return januarySecond.toISOString().split("T")[0];
  };
  ...
  useEffect(() => {
    ...
    // Auto-select January 2nd, 2026 if no date is selected
    const januarySecond = new Date(2026, 0, 2); // January 2nd, 2026 (month is 0-indexed)
    januarySecond.setHours(0, 0, 0, 0);
    const januarySecondDateString = januarySecond.toISOString().split("T")[0];

    const dateToSelect = formData.selectedDate || januarySecondDateString;
    ...
    if (dateToSelect) {
      ...
      // Split the date string and create Date object in local timezone
      const [year, month, day] = dateToSelect.split("-").map(Number);
    ...
    // If we auto-selected January 2nd, 2026, update formData
    if (!formData.selectedDate && dateToSelect === januarySecondDateString) {
      setFormData({
        ...formData,
        selectedDate: januarySecondDateString,
      });
    }
  ...
  const getMondayOfWeek = (date: Date) => {
    ...
    // Normalize to local timezone by setting hours to 0 to avoid timezone issues
    d.setHours(0, 0, 0, 0);
    ...
    // Ensure the result is also normalized to local timezone
    d.setHours(0, 0, 0, 0);
  ...
  const [currentWeekStart, setCurrentWeekStart] = useState(() => {
    ...
    // Set January 2nd, 2026 as the minimum available date and default week
    const januarySecond = new Date(2026, 0, 2); // January 2nd, 2026 (month is 0-indexed)
    januarySecond.setHours(0, 0, 0, 0);
    const januarySecondWeekMonday = getMondayOfWeek(januarySecond);

    // Default to January 2nd, 2026 week
    return januarySecondWeekMonday;
  });
  ...
  useEffect(() => {
    ...
    const loadBookedSlots = async () => {
      try {
        ...
        // The currentWeekStart is already Monday, so we need to get the exact week range
        const fromDateVisible = new Date(currentWeekStart);
        const toDateVisible = new Date(currentWeekStart);
        toDateVisible.setDate(fromDateVisible.getDate() + 6); // 7 days from start (Monday to Sunday)

        // Format dates for API call - ensure we're getting the correct week range
        // Use local date formatting to avoid timezone issues
        const fromDate =
          fromDateVisible.getFullYear() +
          "-" +
          String(fromDateVisible.getMonth() + 1).padStart(2, "0") +
          "-" +
          String(fromDateVisible.getDate()).padStart(2, "0") +
          "T00:00:00.00";
        const toDate =
          toDateVisible.getFullYear() +
          "-" +
          String(toDateVisible.getMonth() + 1).padStart(2, "0") +
          "-" +
          String(toDateVisible.getDate()).padStart(2, "0") +
          "T23:59:59.99";
        ...
        console.log("Loading booked slots for week:", {
          ...
          fromDateVisible: fromDateVisible.toISOString().split("T")[0],
          toDateVisible: toDateVisible.toISOString().split("T")[0],
          weekStartDay: currentWeekStart.getDay(), // Should be 1 for Monday
  ...
  const generateDates = useCallback(() => {
    ...
    // Set January 2nd, 2026 as the minimum available date
    const januarySecond = new Date(2026, 0, 2); // January 2nd, 2026 (month is 0-indexed)
    januarySecond.setHours(0, 0, 0, 0);
    ...
    for (let i = 0; i < 84; i++) {
      ...
      dates.push({
        ...
        fullDate: fullDate,
        dayKey: dayKey,
        available: loopDate >= januarySecond, // Changed from today to januarySecond
      });
      ...
      if (i < 7) {
        console.log("Generated date:", {
          ...
          parsedDayOfWeek: new Date(fullDate).getDay(),
        });
      }
    }
  ...
  const navigateWeek = (direction: "prev" | "next") => {
    ...
    // Don't go before January 2nd, 2026 week
    const januarySecond = new Date(2026, 0, 2); // January 2nd, 2026 (month is 0-indexed)
    januarySecond.setHours(0, 0, 0, 0);
    const januarySecondWeekMonday = getMondayOfWeek(januarySecond);
    ...
    const januarySecondWeekMondayNormalized = new Date(januarySecondWeekMonday);
    januarySecondWeekMondayNormalized.setHours(0, 0, 0, 0);

    if (
      direction === "prev" &&
      newDateNormalized < januarySecondWeekMondayNormalized
    ) {
      // If navigating back would go before January 2nd, 2026 week, don't allow it
      return;
    }
  ...
  const getTimeSlotsForDate = async (dateObj: any, locationId: number) => {
    ...
    try {
      ...
      console.log("Getting time slots for date:", {
        ...
        allBookedSlots: bookedSlots.map((slot) => ({
          SlotDate: slot.SlotDate,
          StartTime: slot.StartTime,
          parsedDate: new Date(slot.SlotDate).toISOString(),
          localDate: new Date(slot.SlotDate).toLocaleDateString(),
        })),
      });
      ...
      // Handle UTC dates from API - extract just the date part for comparison
      const bookedSlotsForDate = bookedSlots.filter((slot) => {
        // Extract date part from API response (format: "2025-11-05T00:00:00")
        const apiDatePart = slot.SlotDate.split("T")[0];

        console.log("Comparing dates:", {
          slotDateFromAPI: slot.SlotDate,
          apiDatePart,
          dateString,
          matches: apiDatePart === dateString,
        });

        return apiDatePart === dateString;
      });
      ...
      console.log("Booked slots for specific date:", {
        ...
        allSlotsComparison: bookedSlots.map((slot) => ({
          originalSlotDate: slot.SlotDate,
          extractedDate: slot.SlotDate.split("T")[0],
          matches: slot.SlotDate.split("T")[0] === dateString,
  ...
  const hasTimeSlotsForDate = (dateObj: any, locationId: number): boolean => {
    if (
      ...
      console.log("hasTimeSlotsForDate - early return:", {
        dateObj: dateObj.fullDate,
        available: dateObj.available,
        locationId,
        loadingDailySlots,
        dailyTimeSlotsLength: dailyTimeSlots.length,
      });
      return false;
    }
    ...
    console.log("hasTimeSlotsForDate check:", {
      dateObj: dateObj.fullDate,
      dayOfWeek,
      locationId,
      relevantSlots: relevantSlots.length,
      allDailySlots: dailyTimeSlots.map((slot) => ({
      ...
      localDate,
      localDateDay: localDate.getDay(),
      oldDateParsed: new Date(dateObj.fullDate),
      oldDateParsedDay: new Date(dateObj.fullDate).getDay(),
    });
        ...
        <div className="flex items-center justify-between mb-4 sm:mb-6">
          <button
            ...
            disabled={(() => {
              ...
              const januarySecondWeekMonday = getMondayOfWeek(januarySecond);
              return (
                currentWeekStart.getTime() <= januarySecondWeekMonday.getTime()
              );
            })()}
          >
